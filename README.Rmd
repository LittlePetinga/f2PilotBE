...
[![R badge](https://img.shields.io/badge/Build%20with-♥%20and%20R-E0218A)](https://cdn.dribbble.com/users/6620596/screenshots/14792345/media/af61fa935b055891cb800a9e41ebb747.gif)
...
* `AUClast()` -- calculates area under the concentration-time curve (AUC) from time zero to the time of the last measurable concentration (AUC~0-t~).

* `Cmax()` -- calculates the maximum observed concentration post-dose (C~max~) directly obtained from the observed concentration-time profile.

* `f2.AUC()` -- calculates the *f*~2~ similarity factor for AUC, from concentration data.

* `f2.Cmax()` -- calculates the *f*~2~ similarity factor for C~max~, from concentration data.

* `f2()` -- calculates the *f*~2~ similarity factor, as proposed by Moore and Flanner in 1996.

* `geomean()` -- calculates geometric mean, i.e., the *N*th root of the product of the *N* observations, equivalent to `exp(mean(log(x)))`.
