...
*f*~2~ based on pharmacokinetic profiles from pilot
...
the use of the geometric mean (G~mean~) *f*~2~ factor, for the
comparison of the absorption rate (given by C~max~) of Test and
Reference formulations. According to the articles, G~mean~ *f*~2~ factor
...
The similarity *f*~2~ factor is a mathematical index widely used to
compare dissolution profiles, evaluating their similarity, using the
percentage of drug dissolved per unit of time.

The similarity *f*~2~ factor, proposed by Moore and Flanner in 1996
...
The *f*~2~ similarity factor ranges from 0 (when
...
[<a href="#references">1</a>]) presents the distribution of *f*~2~
similarity factor as a function of mean difference. Form the *f*~2~
equation, an average difference of [10%]{style="color: red;"},
[15%]{style="color: green;"}, and [20%]{style="color: blue;"} from all
measured time points results in a *f*~2~ value of
...
factor *f*~2~ can be applied as an alternative to the average
...
The similarity *f*~2~ factor is calculated as
...
are not within [80.00–125.00]%, the alternative G~mean~ *f*~2~ factor
...
1.  If the *f*~2~ factor is above or equal to **35** (corresponding to a
...
2.  If the *f*~2~ factor is above or equal to **41** (corresponding to a
...
3.  If the *f*~2~ factor is above or equal to **50** (corresponding to a
...
calculation of the similarity *f*~2~ factor:
...
-   `f2.AUC()` -- calculates the *f*~2~ similarity factor for AUC, from
    concentration data.

-   `f2.Cmax()` -- calculates the *f*~2~ similarity factor for C~max~,
    from concentration data.

-   `f2()` -- calculates the *f*~2~ similarity factor, as proposed by
...
[<a href="#references">1</a>,<a href="#references">2</a>], the *f*~2~
...
The `f2.Cmax()` function allows to calculate the *f*~2~ similarity
...
The *f*~2~ similarity factor for C~max~ can be calculated from the mean
...
2.  Henriques, S.C.; Paixão, P.; Almeida, L.; Silva, N.E. (2023).
    ...
    *f*~2~ Similarity Factor Method. *Pharmaceutics*. *15*(10), 2498.
